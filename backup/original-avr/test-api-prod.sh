...
# Script for testing Helcim API authentication in production environment
echo "=== Helcim API Test - Production Environment ==="
echo "Testing API with different authentication headers"
echo ""

HOST="https://avrnpo.org"
...
echo ""
echo "Writing complete test output to test-results-prod.json"
# Second request with file output and debug info
SAVE_RESPONSE=$(curl -v "${HOST}${ENDPOINT}" \
  -H "X-Debug-Access: ${DEBUG_KEY}" -o test-results-prod.json 2>&1)
...
if [ -f "test-results-prod.json" ]; then
  FILE_SIZE=$(wc -c < test-results-prod.json)
  ...
  if [ $FILE_SIZE -eq 0 ]; then
    ...
    head -c 100 test-results-prod.json
  fi
else
